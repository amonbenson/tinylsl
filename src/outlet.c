...
int lsl_outlet_create(lsl_outlet_t *outlet, const lsl_outlet_config_t *config, uint8_t *sample_buffer, size_t sample_buffer_len) {
    ...
    TRY_OR_RETURN(TRY_ASSERT(config->channel_info.num_channels > 0), "Invalid number of channels: %lu", config->channel_info.num_channels);
    TRY_OR_RETURN(TRY_ASSERT(config->channel_info.format >= 0 && config->channel_info.format < 8), "Invalid channel format: %d", config->channel_info.format);
    TRY_OR_RETURN(TRY_ASSERT(config->channel_info.order == LSL_LSB_FIRST || config->channel_info.order == LSL_MSB_FIRST), "Invalid byte order: %d", config->channel_info.order);
    ...
    TRY_OR_RETURN(TRY_ASSERT(sample_buffer_len >= required_buffer_len), "Sample buffer is not large enough (required at least %lu bytes, but was %lu)", required_buffer_len, sample_buffer_len);
    TRY_OR_RETURN(TRY_ASSERT(sample_buffer != NULL), "Sample buffer must not be NULL.");
...
int lsl_outlet_push_current_sample(lsl_outlet_t *outlet) {
    TRY_OR_RETURN(EMIT(&outlet->callbacks, sample_available, outlet, lsl_outlet_current_sample(outlet)), "Failed to invoke sample_available callback");

    return 0;
}

int lsl_outlet_set_channel_value(lsl_outlet_t *outlet, size_t channel, lsl_sample_value_t value) {
    TRY_OR_RETURN(TRY_ASSERT(channel < outlet->config.channel_info.num_channels), "Channel index %zu out of range.", channel);
...
int lsl_outlet_set_channel_value_float(lsl_outlet_t *outlet, size_t channel, float value) {
    TRY_OR_RETURN(TRY_ASSERT(outlet->config.channel_info.format == LSL_FLOAT32), "Incompatible channel format %d.", outlet->config.channel_info.format);
...
int lsl_outlet_set_channel_value_double(lsl_outlet_t *outlet, size_t channel, double value) {
    TRY_OR_RETURN(TRY_ASSERT(outlet->config.channel_info.format == LSL_DOUBLE64), "Incompatible channel format %d.", outlet->config.channel_info.format);
...
int lsl_outlet_set_channel_value_int32(lsl_outlet_t *outlet, size_t channel, int32_t value) {
    TRY_OR_RETURN(TRY_ASSERT(outlet->config.channel_info.format == LSL_INT32), "Incompatible channel format %d.", outlet->config.channel_info.format);
...
int lsl_outlet_set_channel_value_int16(lsl_outlet_t *outlet, size_t channel, int16_t value) {
    TRY_OR_RETURN(TRY_ASSERT(outlet->config.channel_info.format == LSL_INT16), "Incompatible channel format %d.", outlet->config.channel_info.format);
...
int lsl_outlet_set_channel_value_int8(lsl_outlet_t *outlet, size_t channel, int8_t value) {
    TRY_OR_RETURN(TRY_ASSERT(outlet->config.channel_info.format == LSL_INT8), "Incompatible channel format %d.", outlet->config.channel_info.format);
...
int lsl_outlet_set_channel_value_int64(lsl_outlet_t *outlet, size_t channel, int64_t value) {
    TRY_OR_RETURN(TRY_ASSERT(outlet->config.channel_info.format == LSL_INT64), "Incompatible channel format %d.", outlet->config.channel_info.format);
